...
(function () {
  ...
  angular
    ...
    .controller('OccupantUserController', OccupantUserController)
    .factory('OccupantUserService', OccupantUserService);

  OccupantUserController.$inject = ['$scope', '$rootScope', 'AuthenticationService', 'logger', 'exception', 'OccupantUserService'];

  function OccupantUserController($scope, $rootScope, AuthenticationService, logger, exception, OccupantUserService) {
    var vm = this;
    ...
    $scope.loadUsers = function (start, number) {
      $scope.isLoading = true;
      OccupantUserService.getOccupants(start, number)
  ...
  function OccupantUserService(requestHandler) {
    var service = {};

    service.getOccupants = function (pageNum, pageSize) {
      var url = appUtil.modifyProductUrl(appEndpoints.FACILITY_MANAGER_ENDPOINTS.users, pageSize, pageNum, 'createdAt', 'DESC', false)+"&userType="+appDetails.USER_TYPES.OCCUPANT;
